...
def main(parser):
    ...
    tr_dl = DataLoader(tr_ds, batch_size=model_config.batch_size, shuffle=True, num_workers=1, drop_last=False)

    val_ds = ChatbotDataset(data_config.validation, tokenizer.list_of_string_to_arr_of_pad_token_ids)
    val_dl = DataLoader(val_ds, batch_size=model_config.batch_size, shuffle=True, num_workers=1, drop_last=False)
    ...
    for epoch in tqdm(range(model_config.epochs), desc='epoch', total=model_config.epochs):
        ...
        for step, mb in tqdm(enumerate(tr_dl), desc='steps', total=len(tr_dl)):
            ...
            # str_input, str_dec = decoding_to_pair(enc_input, dec_input, tokenizer)
            # neg_inputs+=str_input
            # neg_samples+=str_pred
            # print(str_input)
            # print('---------------------------------------')
            # print(enc_input.shape, y_pred_copy.shape)
            # print(str_pred)
            # print('---------------------------------------')
            # print(dec_input)
            # return
